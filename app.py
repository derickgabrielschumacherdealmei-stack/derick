...
if prompt := st.chat_input("Escreve a tua mensagem aqui..."):
    ...
    # Respostas diretas e naturais, sem introduções robóticas desnecessárias
    if "Gaúcho" in opcao_voz:
        lang_code = 'pt'
        resposta_ia = f"Bah, guri! Tchê, {prompt}"
    elif "Mineiro" in opcao_voz:
        lang_code = 'pt'
        resposta_ia = f"Uai, sô! {prompt}"
    elif "Carioca" in opcao_voz:
        lang_code = 'pt'
        resposta_ia = f"Papo reto, irmão: {prompt}"
    elif "Italiano" in opcao_voz:
        lang_code = 'it'
        resposta_ia = f"{prompt}"
    ...
    with st.chat_message("assistant"):
        ...
        # Gerar o áudio com gTTS de forma limpa e segura
