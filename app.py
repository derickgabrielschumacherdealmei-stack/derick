...
# --- 2. CORREÇÃO DE COMPATIBILIDADE JAVASCRIPT + ESTILOS ---
st.markdown("""
<script>
    // Polyfill para navegadores antigos que não suportam .toSorted() (Corrige o erro do Streamlit)
    if (!Array.prototype.toSorted) {
        Array.prototype.toSorted = function(compareFn) {
            return [...this].sort(compareFn);
        };
    }
    if (!Object.keys(Object.prototype).toSorted && typeof Object.toSorted !== 'function') {
        // Garante compatibilidade geral para objetos se necessário
    }
</script>
...
# --- 3. GESTÃO DE ESTADO DO SOTAQUE/IDIOMA ---
if "opcao_voz" not in st.session_state:
    st.session_state.opcao_voz = "Brasileiro - Ana"

# --- 4. BARRA LATERAL ---
st.sidebar.markdown("<h2 style='color: #ff3333; margin-bottom: 2px; font-size: 20px;'>🔥 IMPOSSÍVEL</h2>", unsafe_allow_html=True)
...
st.markdown("---")
st.markdown(f"### 💬 Conversa Ativa <span style='font-size: 13px; color: #ff3333;'>({st.session_state.opcao_voz})</span>", unsafe_allow_html=True)

# --- 6. HISTÓRICO E MOTOR DE CHAT ---
